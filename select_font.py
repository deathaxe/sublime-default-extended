...
# class SelectViewFontCommand(sublime_plugin.WindowCommand):
...
#         fonts = view.settings().get('fonts')
#         if not fonts:
#             fonts = [view.settings().get("font_face")]

#         fonts.insert(0, "Global Font")

#         def on_done(index: int) -> None:
#             if index == 0:
#                 view.settings().erase('font_face')
#             elif index > 0:
#                 view.settings().set('font_face', fonts[index])

#         self.window.show_quick_panel(fonts, on_done)
